let state = {
  ...
  actionHistory: [],
  baseDomain: new URL(window.location.href).hostname // Track base domain
};
    ...
    if (typeof chrome !== 'undefined' && chrome.storage && chrome.storage.local) {
      ...
      if (result.navState) {
        ...
        if (savedDomain && currentDomain && savedDomain === currentDomain) {
          // Restore state since we're on the same domain
          ...
          // Update current page but keep everything else
          state.currentPage = window.location.href;
          await saveState();
          
          // If navigation was active, resume it after page fully loads
          if (state.isActive && state.goal) {
            console.log('[Content Script] Resuming navigation on new page...');
            // Wait for page to be fully interactive
            if (document.readyState === 'complete') {
              resumeNavigation();
            } else {
              window.addEventListener('load', resumeNavigation);
            }
          }
        } else {
          // Different domain, reset state
...
// Resume navigation after page load
...
async function onElementInteraction(e) {
  ...
  // Don't clear completed elements anymore - let them persist
  console.log('[Element Interaction Complete] Step:', state.currentStep);

  await saveState();
  
  // Check if page changed (navigation occurred)
  const newUrl = window.location.href;
  if (newUrl !== state.currentPage) {
    console.log('[Page will change] Waiting for new page to load...');
    // State will be restored automatically by loadState() on new page
  } else {
    // Same page, continue immediately
    await highlightNextElement();
  }
}
